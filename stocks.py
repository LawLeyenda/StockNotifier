...
class Stocks:
    ...
    def price(self, ticker):
        try:
            ...
            if(self.myDatabase.sql_fetch_one)
            self.myDatabase.sql_insert_stock()
            ...
            try:
                ...
                self.myDatabase.at["price", ticker] = 0.0
    ...
    def update_add_stock(self, ticker):
        ...
        if ticker not in self.myDatabase:
            self.myDatabase[ticker] = ""
        self.myDatabase.at["price", ticker] = stock_quote.get('latestPrice')
        self.myDatabase.at["yesterday_close", ticker] = stock_quote.get('previousClose')
        self.myDatabase.at['company_name', ticker] = stock_quote.get('companyName')
        self.myDatabase.at["pe_ratio", ticker] = stock_quote.get('peRatio')
        self.myDatabase.at["week52High", ticker] = stock_quote.get('week52High')
        self.myDatabase.at["week52Low", ticker] = stock_quote.get('week52Low')
        StockData.save(self.myDatabase, "myStockData.csv")

    '''add stock/update'''

    def remove_stock(self, ticker):
        self.myDatabase = self.myDatabase.drop([ticker], axis=1)
        StockData.save(self.myDatabase, "myStockData.csv")
    ...
    def notify_user(self, ticker):
        price = self.myDatabase.at['price', ticker]
        numerator = price - self.myDatabase.at['yesterday_close', ticker]
        name = self.myDatabase.at['company_name', ticker]
        change = round(numerator / self.myDatabase.at['yesterday_close', ticker] * 100, 2)  # change is as 1s digit
        if self.myDatabase.at['user_notified?', ticker] != 1:  # remember to reset to 0 at the end of a trading day
            if change >= 2.5 or change <= -2.5:
                self.myDatabase.at[
                    'user_notified?', ticker] = 1  # set notification to true for further notification constraints
                self.myDatabase.at['user_notified_price', ticker] = price
                time_now = datetime.now()

                self.myDatabase.at['user_notified_time', ticker] = time_now - timedelta(hours=5)
                self.myDatabase.at['user_notified_percent', ticker] = change
    ...
    def renotify(self, ticker):
        if self.myDatabase.at['user_notified?', ticker] == 1:
            price = self.myDatabase.at['price', ticker]
            notified = self.myDatabase.at['user_notified_percent', ticker]
            numerator = price - self.myDatabase.at['yesterday_close', ticker]
            name = self.myDatabase.at['company_name', ticker]
            change = round((numerator / self.myDatabase.at['yesterday_close', ticker]) * 100, 2)
            time_now = datetime.now()
            if (notified > 0 and change >= 1.5 + notified) or \
                    (notified < 0 and change <= -1.5 + notified) and \
                    (int((time_now - self.myDatabase.at[
